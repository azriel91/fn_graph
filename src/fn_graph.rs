...
use crate::{
    ...
    StreamOutcomeState, StreamProgressState,
};
...
impl<F> FnGraph<F> {
        ...
        F: 'f,
    {
        ...
        let scheduler = async move {
            ...
            let fold_stream_state = poll_and_track_fn_ready(fn_ready_rx, &mut fn_ids_processed)
        ...
        let (stream_progress_state, (seed, fn_ids_processed)) = futures::join!(queuer, scheduler);
        let stream_outcome = StreamOutcome::new(
            &graph_structure,
            (),
        ...
        F: 'f,
    {
        ...
        let scheduler = async move {
            let mut fn_ids_processed = Vec::with_capacity(graph_structure.node_count());
            poll_and_track_fn_ready(fn_ready_rx, &mut fn_ids_processed)
        ...
        let (stream_progress_state, fn_ids_processed) = futures::join!(queuer, scheduler);
        let stream_outcome = StreamOutcome::new(
            &graph_structure,
            (),
            stream_progress_state,
        ...
        F: 'f,
    {
        ...
        let scheduler = async move {
            let mut fn_ids_processed = Vec::with_capacity(graph_structure.node_count());
            let seed_result = poll_and_track_fn_ready(fn_ready_rx, &mut fn_ids_processed)
                .map(Result::<_, E>::Ok)
        ...
        let (stream_progress_state, (seed_result, fn_ids_processed)) =
...
fn stream_setup_init_concurrent<'f>(
    ...
    edge_counts: &EdgeCounts,
    opts: StreamOpts<'f, 'f>,
) -> StreamSetupInitConcurrent<'f, impl Future<Output = StreamProgressState> + 'f> {
    let StreamOpts {
        stream_order,
        #[cfg(feature = "interruptible")]
        interruptibility_state,
...
async fn fn_ready_queuer<'f>(
    ...
    let mut fn_ready_tx = Some(fn_ready_tx);
    if fns_remaining == 0 {
        stream_progress_state = StreamProgressState::Finished;
        fn_ready_tx.take();
    }
